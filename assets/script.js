...
// ... (Mantenha Utils, Config e Modal iguais ao anterior) ...
...
const renderAvatar = (el, { user, avatar_url }) => {
    if (!el) return;
    el.innerHTML = "";
    if (avatar_url) { el.style.backgroundImage = `url(${avatar_url})`; } else { el.style.backgroundImage = 'none'; el.textContent = escape((user || "?").slice(0, 2).toUpperCase()); }
};
const modal = ({ title, val = '', placeholder = '', onSave, isPassword = false }) => {
    ...
    if (!passInput) { passInput = document.createElement('input'); passInput.id = 'modal-pass-input'; passInput.type = 'password'; passInput.className = 'input'; passInput.style.width = '100%'; passInput.style.marginBottom = '10px'; input.parentNode.insertBefore(passInput, input); }
    if (isPassword) { input.style.display = 'none'; input.required = false; passInput.value = val; passInput.placeholder = placeholder; passInput.style.display = 'block'; passInput.required = true; passInput.focus(); } else { input.style.display = 'block'; input.required = true; passInput.style.display = 'none'; passInput.required = false; input.focus(); }
    view.hidden = false;
    const form = $('#modal-form');
    const newForm = form.cloneNode(true);
    form.parentNode.replaceChild(newForm, form);
    newForm.onsubmit = (e) => { e.preventDefault(); const valToSave = isPassword ? $('#modal-pass-input').value.trim() : input.value.trim(); if (valToSave) onSave(valToSave); view.hidden = true; };
    $('#modal-cancel-btn').onclick = () => view.hidden = true;
};

// --- API Layer ---
const api = {
    async get(endpoint) { try { const res = await fetch(endpoint); if (!res.ok) throw new Error(`API Error: ${res.status}`); return await res.json(); } catch (err) { console.error(err); return null; } },
    async post(endpoint, body) { try { const headers = {}; if (!(body instanceof FormData)) { headers['Content-Type'] = 'application/json'; body = JSON.stringify(body); } const res = await fetch(endpoint, { method: 'POST', headers, body }); if (!res.ok) { const err = await res.json(); throw new Error(err.error || 'Erro na requisição'); } return await res.json(); } catch (err) { console.error(err); toast(err.message, 'error'); return null; } },
    
    // Helper Genérico para Upload (Avatar ou Capa)
    async uploadImage(type, file) {
        const formData = new FormData();
        formData.append(type, file); // 'avatar' ou 'cover'
...
// --- Actions ---
const actions = {
    async loadFeed() { const data = await api.get(`/api/posts?user=${encodeURIComponent(state.user)}`); if (data) ui.renderPosts($('#posts'), data.posts || []); },
    async loadExplore() { ui.switchView('explore'); const data = await api.get('/api/posts/explore'); if (data) ui.renderPosts($('#explore-posts'), data.posts || []); },
    async createPost() { const input = $('#feedInput'); const fileInput = $('#feedImageInput'); const text = input.value.trim(); if (!text) return; $('#feedSend').disabled = true; $('#feedSend').textContent = "Enviando..."; const formData = new FormData(); formData.append('user', state.user); formData.append('text', text); if (fileInput.files.length > 0) formData.append('image', fileInput.files[0]); const res = await api.post('/api/posts', formData); $('#feedSend').disabled = false; $('#feedSend').textContent = "Publicar"; if (res) { input.value = ""; fileInput.value = ""; $('#image-preview-container').style.display = 'none'; actions.loadFeed(); toast("Post publicado!", "success"); } },
    ...
    async loadProfile(username) {
        ...
        // Reseta visual antes de carregar
        renderAvatar($('#profileAvatar'), { user: username });
        $('#profileCover').style.backgroundImage = 'none'; // Limpa capa anterior
        ...
        if (data) {
            ...
            api.get(`/api/profile/${encodeURIComponent(username)}/vibe`).then(v => { if (v && v.vibe) { $('#profileVibeText').textContent = v.vibe.message; $('#profileVibe').style.borderLeftColor = v.vibe.color; $('#profileVibe').hidden = false; } });
            ...
            if (isOwner) {
                ...
                api.get(`/api/isfollowing/${encodeURIComponent(username)}?follower=${encodeURIComponent(state.user)}`).then(f => { if (f && f.isFollowing) { $('#editBioBtn').textContent = "Deixar de Seguir"; $('#editBioBtn').onclick = () => actions.toggleFollow(username, false); } else { $('#editBioBtn').onclick = () => actions.toggleFollow(username, true); } });
            }
        }
    ...
    async toggleFollow(target, isFollowing) { const endpoint = isFollowing ? '/api/follow' : '/api/unfollow'; const res = await api.post(endpoint, { follower: state.user, following: target }); if (res) { toast(isFollowing ? `Seguindo ${target}` : `Deixou de seguir ${target}`, "success"); actions.loadProfile(target); } },
    async vote(type) { const btn = $(`button[data-rating="${type}"]`); const isActive = btn.classList.contains('active') || btn.classList.contains('active-negative'); const endpoint = isActive ? '/api/profile/unrate' : '/api/profile/rate'; const res = await api.post(endpoint, { from_user: state.user, to_user: state.viewedUser, rating_type: type }); if (res) { actions.loadProfile(state.viewedUser); toast(isActive ? "Voto removido" : "Voto enviado!", "success"); } },
    async loadCommunity(id) { state.communityId = id; ui.switchView('community'); const details = await api.get(`/api/community/${id}/details`); if (details) { $('#community-name-channel').textContent = details.community.name; $('#community-avatar-channel').textContent = details.community.emoji; const isOwner = details.community.owner_user === state.user; $('#btn-edit-community').hidden = !isOwner; actions.loadTopics(id); actions.loadMembers(id); } },
    ...
    async editBio() {
        modal({
            ...
            onSave: async (bio) => {
                // Agora aceita bio vazia ou qualquer texto
    ...
    async updateMood() { modal({ title: "Novo Mood", val: $('#userbar-mood').textContent, onSave: async (m) => { const res = await api.post('/api/profile/mood', { user: state.user, mood: m }); if(res) { $('#userbar-mood').textContent = res.mood; if($('#profileMood')) $('#profileMood').textContent = `Mood: ${res.mood}`; toast("Mood atualizado!", "success"); } }}); },
    startDM(target) { if (target === state.user) return; state.currentChannel = [state.user, target].sort().join('_'); ui.switchView('chat'); $('#messages').innerHTML = ""; $('#topic').textContent = `@ ${target}`; socket.emit('joinChannel', { channel: state.currentChannel, user: state.user }); }
};

// ... (Mantenha ui, renderPosts, renderRatings... iguais ao anterior) ...
const ui = {
    switchView(viewName) { $$('.app > section, .app > main').forEach(el => el.hidden = true); const app = $('.app'); app.classList.remove('view-home', 'view-community'); $$('.server, .add-btn, .pill').forEach(b => b.classList.remove('active')); if (['feed', 'explore', 'profile', 'explore-servers'].includes(viewName)) { app.classList.add('view-home'); $('.header').hidden = false; $('.channels').hidden = true; if (viewName === 'feed') $('#home-btn').classList.add('active'); if (viewName === 'explore') $('#btn-explore').classList.add('active'); if (viewName === 'explore-servers') $('#explore-servers-btn').classList.add('active'); } else if (viewName === 'community' || viewName === 'chat') { app.classList.add('view-community'); $('.header').hidden = true; $('.channels').hidden = false; if (viewName === 'community') { $(`.community-btn[data-community-id="${state.communityId}"]`)?.classList.add('active'); $('#view-community-topics').hidden = false; return; } } const map = { 'feed': 'view-feed', 'explore': 'view-explore', 'profile': 'view-profile', 'explore-servers': 'view-explore-servers', 'chat': 'view-chat' }; if (map[viewName]) $(`#${map[viewName]}`).hidden = false; },
    renderPosts(container, posts) { container.innerHTML = posts.length ? "" : "<div class='meta p-4'>Nada por aqui ainda.</div>"; posts.forEach(p => { const node = document.createElement("div"); node.className = "post"; const date = new Date(p.timestamp).toLocaleString('pt-BR'); const editBtn = p.user === state.user ? `<button class="mini-btn" onclick="editPost(${p.id})">Editar</button>` : ''; const imageHtml = p.image_url ? `<div style="text-align: center; margin-top: 8px;"><img src="${p.image_url}" alt="Imagem do post" class="post-image" onclick="window.open(this.src)"></div>` : ''; node.innerHTML = `<div class="avatar-display post-avatar"></div><div style="width: 100%;"><div class="meta"><strong class="post-username" data-u="${escape(p.user)}">${escape(p.user)}</strong> • ${date}</div><div id="post-text-${p.id}">${escape(p.text)}</div>${imageHtml}<div class="post-actions"><button class="mini-btn" onclick="likePost(${p.id})">❤ ${p.likes || 0}</button><button class="mini-btn" onclick="commentPost(${p.id})">Comentar</button>${editBtn}</div><div class="comments" id="comments-${p.id}"></div></div>`; renderAvatar(node.querySelector('.avatar-display'), p); node.querySelector('.post-username').onclick = () => actions.loadProfile(p.user); container.appendChild(node); api.get(`/api/posts/${p.id}/comments`).then(d => { if(d && d.comments.length) { $(`#comments-${p.id}`).innerHTML = d.comments.map(c => `<div class="meta"><strong>${escape(c.user)}</strong>: ${escape(c.text)}</div>`).join(''); }}); }); },
...
const bindEvents = () => {
    ...
    // 👇 BIND DE UPLOAD AGORA USA A FUNÇÃO CORRETA 👇
