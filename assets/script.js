...
const toast = (msg, type = 'info') => {
    ...
    setTimeout(() => {
        ...
        // Garante a remoção mesmo se a animação falhar
...
const modal = ({ title, val = '', placeholder = '', onSave, isPassword = false }) => {
    ...
    // Cria/Reseta input de senha se necessário
    ...
    if (isPassword) {
       ...
       passInput.value = ""; // Limpa senha anterior por segurança
       passInput.placeholder = placeholder;
       passInput.style.display = 'block';
       passInput.focus();
    } else {
       input.style.display = 'block';
       input.value = val; // Define o valor ATUAL (ex: bio atual)
    ...
    // Isso impede que o botão "Salvar" dispare múltiplas vezes
    ...
    newForm.onsubmit = (e) => {
        ...
        // Se for texto normal (Bio), usa o .value do textarea
        // Se for senha, usa o input de senha
        const inputEl = isPassword ? newForm.querySelector('#modal-pass-input') : newForm.querySelector('#modal-input');
        const valToSave = inputEl ? inputEl.value.trim() : ""; // Permite vazio (para limpar bio)
...
const actions = {
    ...
    async editBio() {
        modal({
            title: "Editar Bio",
            val: $('#profileBio').textContent, // Pega a bio atual
            onSave: async (bio) => {
