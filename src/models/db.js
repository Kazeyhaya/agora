...
async function setupDatabase() {
  ...
  try {
    ...
    // Posts (Agora com suporte a imagens)
    ...
    // --- 2. MIGRAÇÃO DA BD (Atualiza tabelas antigas sem perder dados) ---
    const migrations = [
        ...
        // 👇 MIGRAÇÃO NOVA: IMAGENS NOS POSTS 👇
        'ALTER TABLE posts ADD COLUMN IF NOT EXISTS image_url TEXT'
    ];

    for (let query of migrations) {
        try { await client.query(query); } catch (e) { /* Ignora se já existir */ }
    }
