...
async function seedDatabase(client) {
  try {
    ...
    if (res.rows.length === 0) {
      const tech = await client.query(`INSERT INTO communities (name, description, emoji, members, owner_user) VALUES ('Tecnologia 💻', 'Dev e Hardware', '💻', 1, 'Admin') RETURNING id`);
      const music = await client.query(`INSERT INTO communities (name, description, emoji, members, owner_user) VALUES ('Música 🎵', 'Pop e Rock', '🎵', 1, 'Admin') RETURNING id`);
