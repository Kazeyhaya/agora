...
class Profile {
    ...
    constructor({ user, bio, mood, avatar_url, cover_url }) {
        ...
        this.cover_url = cover_url || null; // <-- NOVO
    }
    ...
    // --- MÉTODOS DE SUPORTE ---
    async follow(target) { await db.query('INSERT INTO follows (follower_user, following_user) VALUES ($1, $2) ON CONFLICT DO NOTHING', [this.user, target]); return true; }
    async unfollow(target) { await db.query('DELETE FROM follows WHERE follower_user = $1 AND following_user = $2', [this.user, target]); return true; }
    ...
    async isFollowing(target) { const r = await db.query('SELECT 1 FROM follows WHERE follower_user = $1 AND following_user = $2', [this.user, target]); return r.rows.length > 0; }
    ...
    static async findByUser(username) {
        ...
        if (result.rows[0]) return new Profile(result.rows[0]);
