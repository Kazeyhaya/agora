...
socket.on('joinChannel', async (data) => {
    ...
    // CORREÇÃO: Pega o nome do canal, não importa se o front
    // mandou um texto (ex: "geral") ou um objeto (ex: { channel: "geral" })
    const channelName = (typeof data === 'object' && data.channel) ? data.channel : data;

    if (!channelName || typeof channelName !== 'string') {
      console.error('Erro: Tentativa de entrar em canal inválido. Dados recebidos:', data);
      return;
    }
